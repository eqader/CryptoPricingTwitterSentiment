...
try:
    with open("restart_"+aws_bucket_path[0:-1]+".txt","r") as file:
...
while i <= max_pages:
    ...
    while retry:
        try:
            response=client.search_all_tweets(query,
                                    ...
                                    next_token=next_token,start_time=tweepy_start_time,end_time=tweepy_end_time,
    ...
    if calls_pulled_per_file >= calls_per_file:
        ...
        with open("restart_"+aws_bucket_path[0:-1]+".txt","w") as file:
            restart_dataout={'next_token':next_token,'files_written':files_written,
                             'tweets_pulled':tweets_pulled}
